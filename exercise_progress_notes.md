...
> 统计时间：`2026-04-27` | 总题数：`40` | 已通过：`40` | 未完成：`0` | 得分：`200 / 200`
>
> 进度：`████████████████████████████████████████` 100%
...
| 20 | `20_mybash` | 系统 | 测试框架实际从 `tests/` 目录执行脚本；参数解析支持引号；对 `/workspace/...` 路径做映射 |
...
| 36 | `36_lru_cache` | 数据结构 | 哈希表负责 O(1) 定位，双向链表维护最近使用顺序；访问命中后要移动到头部 |
...
### 16 mysed / 17 myfile / 18 mywc / 19 mytrans / 20 mybash

> 逻辑较直接，参见总览表格中的核心要点即可。

`20_mybash` 这题额外要注意两件事：

- 测试框架不是在练习目录里执行脚本，而是在 `tests/` 目录下执行 `bash ./test_20_mybash.sh`
- 测试脚本里的路径写死成 `/workspace/exercises/20_mybash/...`，当前环境不是这个绝对路径时要做兼容映射

本题最终通过点：

- `Makefile` 要能递归构建 `mysed` / `myfile` / `mywc` / `mytrans` / `mybash`
